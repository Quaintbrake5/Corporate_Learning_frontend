...
const CourseManagement: React.FC = () => {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getCourses(1, 100) as { items: Course[] };
      setCourses(data?.items || (Array.isArray(data) ? data : []));
    } catch (err: unknown) {
      setError(parseErrorMessage(err, 'Failed to load courses'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (course: Course) => {
    ...
    try {
      ...
      setError(parseErrorMessage(err, 'Failed to delete course'));
    }
  };
